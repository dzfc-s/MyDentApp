...
class AuthProvider extends ChangeNotifier {
  ...
  // 10.0.2.2 is the standard Android emulator alias for the host machine's localhost — works
  // out of the box with zero extra setup. Override with --dart-define=API_BASE_URL=... for a
  // physical device (USB via `adb reverse tcp:5126 tcp:5126` + 127.0.0.1, or WiFi with the PC's
  // LAN address).
  ...
  Future forgotPassword(String email) async {
    var uri = Uri.parse("${_baseUrl}Access/ForgotPassword");
    var response = await http.post(uri,
        headers: createHeaders(), body: jsonEncode({"email": email}));
    validateResponse(response);
  }
  ...
  Future resetPassword({
    ...
    var uri = Uri.parse("${_baseUrl}Access/ResetPassword");
    var response = await http.post(
      uri,
