...
namespace MyDent.Services.Validators
{
    ...
    {
        // Assets are only ever used as images in this app (doctor photos, news images, profile
        // pictures) — an allow-list of image types, each checked against the file's actual first
        // bytes rather than trusting the client-supplied ContentType string alone.
        ...
        public AssetInsertValidator()
        {
            ...
            // Only run once ContentType/Base64Content individually pass — an unrecognized
            // ContentType or unparsable Base64 would otherwise throw here instead of failing
            // validation cleanly.
            RuleFor(x => x)
        ...
        private static bool HaveContentMatchingItsDeclaredType(AssetInsertRequest request)
        {
            byte[] bytes;
            try
            {
